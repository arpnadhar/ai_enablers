...
def trf_modnet(root, resume=True, std=0):
    save_model_dir = 'savemodel'
    if resume:
        listfiles = sorted(os.listdir(os.path.join('.',save_model_dir)))

        if (len(listfiles) != 0):
            VModel=listfiles[-1]
            pretrained_ckpt = os.path.join(save_model_dir, VModel)
        else:
            pretrained_ckpt = os.path.join('.','models','modnet','pretrained','modnet_webcam_portrait_matting.ckpt')
    else:
        pretrained_ckpt = os.path.join('.','models','modnet','pretrained','modnet_webcam_portrait_matting.ckpt')
